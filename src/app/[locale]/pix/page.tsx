...
const SUB_PAGES = [
  {
    ...
    desc: 'Create PIX QR codes instantly — works with all Brazilian banking apps',
    soon: false,
  },
  {
    href: '/pix/guide',
    ...
    desc: 'EMV TLV payload spec, CRC16, key types, BCB integration docs',
    soon: true,
  },
];
          ...
          {SUB_PAGES.map(({ href, icon, title, desc, soon }) => (
            <div key={href} className="relative">
              {soon ? (
                ...
                <Link
                  href={href as `/${string}`}
                  className="block bg-white border border-slate-100 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-emerald-100 hover:-translate-y-0.5 transition-all duration-200"
                >
                  <SubPageCardInner icon={icon} title={title} desc={desc} />
                </Link>
              )}
