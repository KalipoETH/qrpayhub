...
import { upiGuideContent } from '@/content/upi/guide';
import type { GuideContent } from '@/content/types';

export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  const { locale } = params;
  const content = upiGuideContent[locale as 'en' | 'de'] ?? upiGuideContent.en;
  return {
...
export default function UPIGuidePage({ params }: { params: { locale: string } }) {
  ...
  const content = upiGuideContent[locale] ?? upiGuideContent.en;
  return (
    <>
      <Script
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-4">
      ...
      <div className="space-y-14 pt-4">
        ...
        {sectionMap['what-is-upi'] && (
          <Section id="what-is-upi" title={sectionMap['what-is-upi'].heading}>
            <Prose>
              <p>{sectionMap['what-is-upi'].content}</p>
            </Prose>
          </Section>
        )}
            ...
            <ol className="space-y-4 mt-4">
              {[
                ...
                },
                {
                  ...
                  body: 'The app activates the camera. The UPI QR can be scanned from a printed slip, a phone screen or a digital PDF.',
                },
                {
                  step: 4,
                  title: 'Payment details are pre-filled',
                  body: 'The payee UPI ID, name, and amount (if embedded) are instantly extracted. No manual entry, no typos, no wrong IFSC codes.',
                },
                {
                  step: 5,
                ...
                },
                {
                  ...
                  body: 'The payer gets a transaction success notification. The payee receives an instant credit alert via SMS and in-app notification.',
                },
              ].map(({ step, title, body }) => (
        ...
        {/* ── Section 3: UPI IDs ───────────────────────────────────────────── */}
        {sectionMap['upi-ids'] && (
          <Section id="upi-ids" title={sectionMap['upi-ids'].heading}>
            <Prose>
              <p>{sectionMap['upi-ids'].content}</p>
            </Prose>
...
{`upi://pay?pa=merchant@okhdfc&pn=Raj%20Stores&am=150.00&cu=INR&tn=Invoice%20001`}
                    ...
                    <tr key={param} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                          required ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-500'
                        }`}>
        ...
        {sectionMap['static-vs-dynamic'] && (
          <Section id="static-vs-dynamic" title={sectionMap['static-vs-dynamic'].heading}>
            <Prose>
              <p>{sectionMap['static-vs-dynamic'].content}</p>
            </Prose>
            <div className="overflow-x-auto rounded-2xl border border-slate-200 mt-3">
              <table className="min-w-full text-sm">
                <thead className="bg-slate-50 text-left">
        ...
        {sectionMap['apps-ecosystem'] && (
          <Section id="apps-ecosystem" title={sectionMap['apps-ecosystem'].heading}>
            <Prose>
              <p>{sectionMap['apps-ecosystem'].content}</p>
            </Prose>
        ...
        {sectionMap['global-expansion'] && (
          <Section id="global-expansion" title={sectionMap['global-expansion'].heading}>
            <Prose>
              <p>{sectionMap['global-expansion'].content}</p>
            </Prose>
        ...
        <div className="bg-orange-50 border border-orange-100 rounded-2xl p-6 text-center space-y-3">
          <p className="font-semibold text-orange-900 text-lg">
            {locale === 'de'
              ? 'Bereit, Ihren UPI QR-Code zu erstellen?'
              : 'Ready to generate your UPI QR Code?'}
          </p>
          <p className="text-orange-700 text-sm">
            {locale === 'de'
              ? 'Kostenlos, sofort, funktioniert mit allen UPI-Apps.'
              : 'Free, instant, works with all UPI apps.'}
          </p>
          <Link
            href="/upi/generator"
