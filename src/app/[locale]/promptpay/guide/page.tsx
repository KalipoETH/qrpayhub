...
import { promptpayGuideContent } from '@/content/promptpay/guide';
import type { GuideContent } from '@/content/types';

export async function generateMetadata({ params }: { params: { locale: string } }): Promise<Metadata> {
  const { locale } = params;
  const content = promptpayGuideContent[locale as 'en' | 'de'] ?? promptpayGuideContent.en;
  return {
...
export default function PromptPayGuidePage({ params }: { params: { locale: string } }) {
  ...
  const content = promptpayGuideContent[locale] ?? promptpayGuideContent.en;
  return (
    <>
      <Script
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-4">
      ...
      <header className="space-y-3 pt-4 pb-6 border-b border-slate-100">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {locale === 'de' ? 'PromptPay FAQ →' : 'PromptPay FAQ →'}
          </Link>
        </div>
      </header>
      ...
      <div className="space-y-14 pt-4">
        ...
        {sectionMap['what-is-promptpay'] && (
          <Section id="what-is-promptpay" title={sectionMap['what-is-promptpay'].heading}>
            <Prose>
              <p>{sectionMap['what-is-promptpay'].content}</p>
            </Prose>
          </Section>
        )}
          ...
          <Section id="how-it-works" title={sectionMap['how-it-works'].heading}>
            <ol className="space-y-4">
              {[
                {
                  step: 1,
                  title: locale === 'de' ? 'Empfänger teilt PromptPay QR oder Schlüssel' : 'Recipient shares PromptPay QR or key',
                  body: locale === 'de'
                    ? 'Der Zahlungsempfänger gibt eine Telefonnummer, nationale Ausweis-ID oder einen PromptPay QR-Code an. Statische QR-Codes funktionieren für beliebige Beträge; dynamische QR-Codes enthalten einen voreingestellten Betrag für Rechnungen und E-Commerce.'
                ...
                {
                  step: 2,
                  title: locale === 'de' ? 'Zahlender öffnet eine thailändische Banking-App' : 'Payer opens any Thai banking app',
                  body: locale === 'de'
                    ? 'Alle 30+ teilnehmenden Banken sind vollständig interoperabel. Eine Zahlung von KBank kommt sofort auf einem SCB-Konto an – keine Bindung an ein bestimmtes Institut.'
                ...
                {
                  step: 3,
                  title: locale === 'de' ? 'QR scannen oder Telefon/ID eingeben' : 'Payer scans QR or enters phone/ID',
                  body: locale === 'de'
                    ? 'Die App aktiviert die Kamera zum QR-Scannen, oder der Zahlende gibt den PromptPay-Schlüssel manuell ein. Der Name des Empfängers wird zur Bestätigung angezeigt.'
                ...
                {
                  step: 4,
                  title: locale === 'de' ? 'Zahlungsdetails auf dem Bildschirm bestätigen' : 'Payment details confirmed on screen',
                  body: locale === 'de'
                    ? 'Der Name des Empfängers (wie bei ITMX registriert) wird angezeigt. Bei einem dynamischen QR ist der Betrag vorausgefüllt. Beim statischen QR muss der Zahlende den Betrag eingeben.'
                ...
                },
                {
                  ...
                  body: locale === 'de'
                    ? 'Authentifizierung erfolgt vollständig in der Banking-App. Der PromptPay QR enthält keine Passwörter, PINs oder sensiblen Bankzugangsdaten.'
                ...
                {
                  step: 6,
                  title: locale === 'de' ? 'Überweisung sofort abgeschlossen' : 'Transfer completes instantly',
                  body: locale === 'de'
                    ? 'Die ITMX-Infrastruktur leitet die Zahlung durch das PromptPay-Netzwerk weiter. Gelder werden in Echtzeit zwischen den beiden Bankkonten abgewickelt – typischerweise innerhalb von 5 Sekunden.'
                ...
                {
                  step: 7,
                  title: locale === 'de' ? 'Beide Parteien erhalten App-Benachrichtigung' : 'Both parties receive app notification',
                  body: locale === 'de'
                    ? "Absender und Empfänger erhalten sofortige Push-Benachrichtigungen. Die App des Zahlenden zeigt eine Quittung mit einer Transaktionsreferenznummer für die Unterlagen."
        ...
        {/* ── Section 3: Key Types ─────────────────────────────────────────── */}
        {sectionMap['keys'] && (
          <Section id="keys" title={sectionMap['keys'].heading}>
            <Prose>
              <p>{sectionMap['keys'].content}</p>
            </Prose>
        ...
        {sectionMap['phone-normalization'] && (
          <Section id="phone-normalization" title={sectionMap['phone-normalization'].heading}>
            ...
            <ul className="space-y-3 mt-2">
              {[
                { icon: '1️⃣', text: locale === 'de' ? 'Lokale thailändische Nummer: 0812345678' : 'Start with a local Thai number: 0812345678' },
                { icon: '2️⃣', text: locale === 'de' ? 'Führende 0 entfernen: 812345678' : 'Remove the leading 0: 812345678' },
                { icon: '3️⃣', text: locale === 'de' ? 'Ländervorwahl 0066 voranstellen: 0066812345678' : 'Prepend the country code prefix 0066: 0066812345678' },
                {
                  icon: '🔁',
                  text: locale === 'de'
                    ? 'Beginnt die Eingabe mit +66 (internationales Format), ersetze das + durch 00: +66812345678 → 0066812345678'
...
{`function normalizePromptPayKey(key: string): string {
  ...
  return digits;
        ...
        {/* ── Section 8: Impact ────────────────────────────────────────────── */}
        {sectionMap['impact'] && (
          <Section id="impact" title={sectionMap['impact'].heading}>
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6 text-center space-y-3">
          <p className="font-semibold text-blue-900 text-lg">
            {locale === 'de'
              ? 'Bereit, Ihren PromptPay QR-Code zu erstellen?'
              : 'Ready to generate your PromptPay QR Code?'}
          </p>
          <p className="text-blue-700 text-sm">
            {locale === 'de'
              ? 'Kostenlos, sofort, funktioniert mit allen thailändischen Banking-Apps. Keine Registrierung erforderlich.'
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            >
              {locale === 'de' ? 'PromptPay FAQ lesen →' : 'Read PromptPay FAQ →'}
            </Link>
          </div>
        </div>
