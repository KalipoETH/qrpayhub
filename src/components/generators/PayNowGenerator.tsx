...
function formatSGD(raw: string): string {
...
const KEY_TYPE_META: Record<PayNowKeyType, { icon: string; labelKey: string }> = {
  mobile: { icon: '📱', labelKey: 'mobile' },
  uen:    { icon: '🏢', labelKey: 'uen' },
  nric:   { icon: '🪪', labelKey: 'nric' },
};

// ── Main Component ─────────────────────────────────────────────────────────────

export default function PayNowGenerator() {
  ...
  // Convert date input (YYYY-MM-DD) to YYYYMMDD for payload
  const expiryForPayload = form.expiryDate ? form.expiryDate.replace(/-/g, '') : undefined;

  const paynowData: PayNowData | null = canGenerate
    ? {
        ...
        expiryDate:   expiryForPayload || undefined,
      }
    : null;

  const currentPayload = paynowData ? generatePayNowPayload(paynowData) : null;
  ...
  const handleBlur = useCallback(
    (field: keyof FormState) => {
      ...
      if (!paynowData) return;
      setErrors(validatePayNowData(paynowData));
    },
    [paynowData],
  );
  ...
  const handleDownload = useCallback(() => {
    ...
    canvasRef.current.toBlob((blob) => {
      ...
      a.href     = url;
      a.download = `paynow-${form.merchantName.replace(/\s+/g, '-').toLowerCase()}.png`;
      a.click();
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-5">
        ...
        >
          <div className="relative">
            <input
              ...
              autoComplete="off"
              spellCheck={false}
              className={`w-full px-4 py-2.5 pr-10 rounded-xl border text-sm font-mono transition-colors focus:outline-none focus:ring-2 focus:ring-red-400 ${
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            role="switch"
            aria-checked={form.editable}
            onClick={() => setForm((f) => ({ ...f, editable: !f.editable }))}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-1 ${
              form.editable ? 'bg-red-500' : 'bg-slate-200'
            }`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${
                form.editable ? 'translate-x-6' : 'translate-x-1'
              }`}
            />
          </button>
          <span className="text-sm text-slate-700">{t('editableLabel')}</span>
        </div>
        ...
        >
          <input
            ...
            className={inputClass(!!touched.expiryDate && !!errors.expiryDate)}
          />
        </FormField>
        ...
        {canGenerate ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
            ...
            <p className="text-sm text-slate-400 max-w-[200px]">
              Enter your PayNow key and payee name to generate the QR code
            </p>
          </div>
        )}
...
function inputClass(hasError: boolean) {
  return `w-full px-4 py-2.5 rounded-xl border text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-red-400 ${
    hasError
      ? 'border-red-300 bg-red-50'
